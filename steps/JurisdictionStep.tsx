...
const JurisdictionGuide: React.FC<{ meta: JurisdictionMeta }> = ({ meta }) => (
    <div className="bg-gradient-to-br from-brand-600 to-indigo-700 rounded-2xl p-6 text-white shadow-lg animate-fadeIn">
...
export const JurisdictionStep: React.FC<StepProps> = ({ data, updateData, onValidationChange }) => {
  const { jurisdiction, asset, projectInfo } = data;
  ...
  useEffect(() => {
    if (jurisdiction.country) {
        ...
        getRegionRecommendations(jurisdiction.country, asset.category)
            .then(regions => {
                setRecommendedRegions(Array.isArray(regions) ? regions : []);
                setIsLoadingRegions(false);
            });
    }
  }, [jurisdiction.country, asset.category]);
  ...
  useEffect(() => {
      if (jurisdiction.country && entityDetails.registrationState) {
          ...
          getSpvRecommendation(jurisdiction.country, entityDetails.registrationState, asset.category, projectInfo)
            .then(rec => {
                setSpvRecommendation(rec);
                setIsSpvRecLoading(false);
            });
      }
  }, [jurisdiction.country, entityDetails.registrationState, asset.category, projectInfo]);
  ...
  const handleAutoFill = async () => {
      ...
      const generatedDetails = await generateEntityDetails(
          ...
          jurisdiction.spvType, 
          asset.assetName || projectInfo.projectName || "Project"
      );
    ...
    <div className="space-y-6 md:space-y-10 animate-fadeIn pb-24">
      <div className="space-y-2">
        ...
        <p className="text-slate-500 text-base md:text-lg max-w-2xl">
          Configure the legal wrapper for your {asset.category} assets. Hover over highlighted terms for definitions.
        </p>
      </div>
              ...
              <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                  <div className="mb-4">
                      <p className="text-sm text-slate-600 mb-3">
                          ...
                          <span className="text-xs text-slate-400 block mt-1">AI Recommendation based on {asset.category} in {selectedCountryMeta?.name}:</span>
                      </p>
                                ...
                                <div className="flex flex-col md:flex-row gap-3">
                                    <div className="flex-1 relative">
                                        <input 
                                            ...
                                            placeholder={`${asset.assetName || projectInfo?.projectName || 'Project'} ${jurisdiction.spvType}`}
